...
class Tensor:
    ...
    def relu(self) -> 'Tensor':
        ...
        Returns:
        Tensor: The result of applying ReLU.
        """
        result = Tensor(np.maximum(0, self.data), requires_grad=self.requires_grad)

        if self.requires_grad:
            def _backward(gradient):
                grad = gradient.data * (self.data > 0).astype(np.float64)
                self.backward(Tensor(grad))
            result._grad_fn = _backward
            result.is_leaf = False
    ...
    def sigmoid(self) -> 'Tensor':
        ...
        Returns:
        Tensor: The result of applying sigmoid.
        """
        sigmoid_data = 1 / (1 + np.exp(-self.data))
        result = Tensor(sigmoid_data, requires_grad=self.requires_grad)
    ...
    def tanh(self) -> 'Tensor':
        ...
        Returns:
        Tensor: The result of applying tanh.
        """
        tanh_data = np.tanh(self.data)
        result = Tensor(tanh_data, requires_grad=self.requires_grad)
    ...
    def softmax(self, axis: int = -1) -> 'Tensor':
        ...
        Parameters:
        axis (int): The axis along which to apply the softmax. Default is -1.

        Returns:
        Tensor: The result of applying softmax.
        """
        exp_data = np.exp(self.data - np.max(self.data, axis=axis, keepdims=True))  # Stability improvement
        ...
        if self.requires_grad:
            def _backward(gradient):
                grad = gradient.data
                jacobian = softmax_data[..., None] * (np.eye(softmax_data.shape[-1]) - softmax_data[..., :, None])
                grad = np.einsum('ij,ijk->ik', grad, jacobian) if axis == -1 else np.einsum('...i,...ij->...j', grad, jacobian)
                self.backward(Tensor(grad))
            result._grad_fn = _backward
            result.is_leaf = False
