...
class Tensor:
    def __init__(
        self, data: Union[np.ndarray, list, float], requires_grad: bool = False
    ):
        """
        ...
        self.grad = (
            None if requires_grad else None
        )  # Gradient of the tensor with respect to some scalar value
    ...
    def __add__(self, other: Union["Tensor", float]) -> "Tensor":
        ...
        other_data = (
            ...
            else np.array(other, dtype=np.float64)
        )
        result = Tensor(
        ...
        if result.requires_grad:
            ...
            def _backward(gradient):
                if self.requires_grad:
                    unbroadcast_grad = _GradientUtils.unbroadcast(
                        gradient.data, self.data.shape
                    )
                    self.backward(unbroadcast_grad)
                if isinstance(other, Tensor) and other.requires_grad:
                    unbroadcast_grad = _GradientUtils.unbroadcast(
                        gradient.data, other.data.shape
                    )
    ...
    def mean(self, axis=None, keepdims=False) -> "Tensor":
        ...
        """
        return self.sum(axis=axis, keepdims=keepdims) / np.prod(
            np.array(self.data.shape)[axis] if axis is not None else self.data.shape
        )
    ...
    def reshape(self, *shape) -> "Tensor":
        ...
        if self.requires_grad:
            ...
            def _backward(gradient):
                self.backward(
                    Tensor(
                        gradient.data.reshape(self.data.shape),
                        requires_grad=self.requires_grad,
                    )
                )
    ...
    def softmax(self, axis=-1) -> "Tensor":
        ...
        result = Tensor(
            exps / np.sum(exps, axis=axis, keepdims=True),
            requires_grad=self.requires_grad,
        )
